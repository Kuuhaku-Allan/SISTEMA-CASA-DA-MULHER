...
**Frontend**
- `equipe-ide.html`: O layout principal, contendo o CodeMirror, explorador, painéis laterais (como *Tarefas Guiadas*) e o modal de Revisão.
- `equipe-ide.css`: Estilização e layout responsivo.
- `equipe-ide.js`: Gerenciador de estado local (`rascunhoAtual` em LocalStorage), manipulador do editor e fetch para API de Envio de PRs. Modelos iniciais e definição do catálogo de **Tarefas Guiadas** também ficam aqui.
...
## 2. Tarefas Guiadas e Modelos

A IDE adota o conceito de "Tarefas Guiadas". A UI do frontend (`equipe-ide.js`) dita o tipo de contribuição e limitações cognitivas, mas **nunca de segurança real**.
- Tarefa = O objetivo do trabalho e o checklist associado.
- Modelo = O payload de arquivos iniciais.
- O Backend apenas armazena esses dados passivamente (DTO) no `README.md` gerado.
...
1. O payload é recebido pelo Controller.
2. É despachado ao `ManualTokenGitHubIdeService`.
3. Os dados de tarefa (`Titulo`, `Tipo`, `Escopo`) e o `Checklist` são apensados no README da contribuição e no corpo do Pull Request.
...
- O catálogo de tarefas está *hardcoded* em JavaScript na Fase 2 para facilitar a usabilidade. O backend apenas confia e sanitiza o título. Numa futura transição de integração de *GitHub Issues* (Fase 3), será recomendado espelhar IDs de Issues reais.
